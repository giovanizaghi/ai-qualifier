...
import {
  ...
  type FitLevel
} from '../prospect-qualifier-core';

describe('Prospect Qualifier Core Functions', () => {
  ...
  describe('getQualificationStats', () => {
    it('should calculate correct statistics', () => {
      const mockResults = [
        { score: 85, fitLevel: 'EXCELLENT' as FitLevel },
        { score: 75, fitLevel: 'GOOD' as FitLevel },
        { score: 45, fitLevel: 'FAIR' as FitLevel },
        { score: 25, fitLevel: 'POOR' as FitLevel },
        { score: 90, fitLevel: 'EXCELLENT' as FitLevel }
      ];
    ...
    it('should handle all poor fits', () => {
      const mockResults = [
        { score: 10, fitLevel: 'POOR' as FitLevel },
        { score: 20, fitLevel: 'POOR' as FitLevel },
        { score: 30, fitLevel: 'POOR' as FitLevel }
      ];
  ...
  describe('Score Edge Cases', () => {
    it('should handle boundary values correctly', () => {
      // Test exact threshold boundaries (accounting for rounding)
      expect(getFitLevel(79)).toBe('GOOD'); // Just below EXCELLENT (no rounding)
      expect(getFitLevel(80)).toBe('EXCELLENT'); // Exact EXCELLENT threshold
      expect(getFitLevel(59)).toBe('FAIR'); // Just below GOOD (no rounding)
      expect(getFitLevel(60)).toBe('GOOD'); // Exact GOOD threshold
      expect(getFitLevel(39)).toBe('POOR'); // Just below FAIR (no rounding)
      expect(getFitLevel(40)).toBe('FAIR'); // Exact FAIR threshold
    });

    it('should handle floating point values with rounding', () => {
      // Test how rounding affects fit levels
      expect(getFitLevel(79.4)).toBe('GOOD'); // Rounds to 79
      expect(getFitLevel(79.5)).toBe('EXCELLENT'); // Rounds to 80
      expect(getFitLevel(59.4)).toBe('FAIR'); // Rounds to 59
      expect(getFitLevel(59.5)).toBe('GOOD'); // Rounds to 60
    });

    it('should be consistent with validation flow', () => {
      // Test that the validation and fit level functions work together
      const testCases = [
        { input: 85.7, expectedScore: 86, expectedFit: 'EXCELLENT' },
        { input: 79.3, expectedScore: 79, expectedFit: 'GOOD' },
        { input: 60.0, expectedScore: 60, expectedFit: 'GOOD' },
        { input: 39.8, expectedScore: 40, expectedFit: 'FAIR' },
        { input: 0.2, expectedScore: 0, expectedFit: 'POOR' }
      ];

      testCases.forEach(({ input, expectedScore, expectedFit }) => {
        const validatedScore = validateScore(input);
        const fitLevel = getFitLevel(validatedScore);
        
        expect(validatedScore).toBe(expectedScore);
        expect(fitLevel).toBe(expectedFit);
      });
    });
  });
});